...
app.get("/auth/discord/callback", async (req, res) => {
  ...
  try {
    ...
    const specialisatieMap = {
      ...
      Regulier: "Ambulance",
      "Lifeliner 1": "Ambulance",
      "OvD-G": "Ambulance",
      Rijkswaterstaat: "Handhaving",
      Boswachter: "Handhaving",
      Burger: "Reserve Burger",
      Meldkamer: "Meldkamer",
    };
    let rol = specialisatieMap[specialisatie] || "Burger";

    // Bereken volgnummer XX
    let bestaandeSpec = users.filter((u) => u.specialisatie === specialisatie)
      .length + 1;
    ...
    const roepnummerMap = {
      ...
      Unmarked: "32-" + xx,
      Zulu: "80-14-" + xx,
      Tankautospuit: "13-21" + xx,
      QRT: "13-22" + xx,
      Ladderwagen: "13-3151",
      Regulier: "13-1" + xx,
      "Lifeliner 1": "13-991",
      "OvD-G": "13-881",
      Rijkswaterstaat: "34-" + xx,
      Boswachter: "33-" + xx,
      "Reserve Burger": "BU00-" + xx,
      Meldkamer: "MK-" + xx,
    };
...
app.post("/api/melding", (req, res) => {
  const user = readFile(USERS_FILE, []).find(
    (u) => u.id === req.cookies.userId
  );
...
// API: melding indelen (alleen meldkamer)
app.post("/api/melding/indelen/:id", (req, res) => {
  const user = readFile(USERS_FILE, []).find(
    (u) => u.id === req.cookies.userId
  );
  if (!user || !user.roles.includes("Meldkamer"))
    return res.status(403).json({ error: "Geen toegang" });

  let meldingen = readFile(MELDINGEN_FILE, []);
  const m = meldingen.find((m) => m.id == req.params.id);
  if (m && m.status === "Open") {
    m.claimedBy = req.body.gebruiker;
    writeFile(MELDINGEN_FILE, meldingen);
  }
  res.json({ ok: true });
});

// API: melding afsluiten (verplaats naar archief)
app.post("/api/melding/close/:id", (req, res) => {
  ...
  if (index !== -1) {
    ...
    const geslotenMelding = meldingen.splice(index, 1)[0];
    geslotenMelding.status = "Gesloten";
    archief.push(geslotenMelding);
